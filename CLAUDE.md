...
**Poster (A4) download** (`src/lib/canvas/poster.ts`, plain Canvas 2D — no library): `renderPosterPng()` draws a fixed 2480×3508px (A4 @ 300dpi) PNG with the business logo, name, and a QR box, invoked from `QrCustomizer.tsx`'s `downloadPoster()`. The logo and QR are fetched as base64 data URLs via Server Actions (`getLogoDataUrl`, `getQrPreview` in `src/lib/actions/qr.ts`) instead of loading the logo's Vercel Blob URL directly into the canvas — a cross-origin image taints the canvas and makes `canvas.toBlob()` throw, so the Server Action re-fetches it server-side and re-encodes it as same-origin base64 first.
